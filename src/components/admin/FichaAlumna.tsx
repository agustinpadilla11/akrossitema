            ...
            <div className="col-span-2 pt-4 border-t border-slate-100">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {['foto_gimnasta', 'foto_dni_frente', 'foto_dni_dorso', 'certificado_medico'].map((docKey) => {
                    ...
                    return (
                      <div key={docKey} className="p-4 bg-slate-50 border border-slate-200 rounded-lg flex flex-col items-center justify-center text-center gap-2">
                        <span className="text-[10px] font-bold uppercase text-slate-500 tracking-widest">{docKey.replace(/_/g, ' ')}</span>
                        {url ? (
                          <a href={url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-purple-600 hover:text-purple-800 bg-purple-50 px-3 py-1 rounded">
                            Ver Archivo
                          </a>
                        ) : (
                          <span className="text-[10px] font-bold text-slate-400">Sin cargar</span>
                        )}
                      </div>
                    );
                  })}
